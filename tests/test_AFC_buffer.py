...
class TestAFCBufferInit:
    ...
    def test_non_switched_type_still_sets_advance_pin_from_unconditional_default(self):
        """advance_pin is read once unconditionally (with a None default)
        before the switched-type block potentially re-reads it; for a
        non-switched buffer only that first read ever runs."""
        buf, afc, printer = _build_real_buffer(
            values={"type": "FPS_PFS", "advance_pin": "P2.0"})
        assert buf.advance_pin == "P2.0"
